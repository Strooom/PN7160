...
void test_dummy() {
    ...
    testTag.setUniqueId(testTagLength, testTagData);
    testTag.dump();
    TEST_ASSERT_EQUAL_STRING("no tag", toString(tagStatus::absent));
    TEST_ASSERT_EQUAL_STRING("new tag", toString(tagStatus::foundNew));
    TEST_ASSERT_EQUAL_STRING("old tag", toString(tagStatus::old));
