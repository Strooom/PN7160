...
//
// Summary :
//   This library implements the NFC Controller Interface (NCI) Version 2.0, which is a communication protocol between NFC hardware and a controlling Device Host
...
class NCI {
  public:
    static void initialize();                                    // See NCI specification V1.0, section 4.1 & 4.2
    static void run();                                           // runs the NCI stateMachine
    static void activate();                                      // moves the StateMachine from Idle to Discover and starts the polling
    static void deActivate(NciRfDeAcivationMode theMode);        // moves the StateMachine from PollActive or WaitingForHostSelect back into Idle
    static nciState getState();

    static TagsPresentStatus getTagsPresentStatus();        // read-only get function for the (private) property
  ...
  private:
    static nciState theState;        // keeps track of the state of the NCI stateMachine - FSM
    static TagsPresentStatus tagsStatus;
    ...
    static uint8_t nmbrOfTags;                               // how many tags are currently present
